...
**Goal:** Add an independent `feathertalk-preprocess` crate for strict 110-point landmark parsing, deterministic face geometry, fixed crop/mask constants, and eight-frame audio-window indices.
...
- Landmark files contain exactly 110 non-empty `x y` points; reject malformed, extra-token, non-finite, negative, invalid-UTF-8, and wrong-count input.
...
- [ ] Write tests for: valid 110 points with blank-line skipping; malformed/extra-token line; 109/111-point counts; `NaN`; negative coordinate; invalid UTF-8; missing file.
...
- [ ] Implement `read_landmarks` with `fs::read`, UTF-8 conversion, one-based line numbers, `split_whitespace`, exactly two float tokens, `is_finite`, non-negative checks, and final count exactly 110.
...
- [ ] Add crate-root API coverage that parses a temporary 110-point file, calls bbox/crop/window APIs, and binds `Landmarks::points()` to `&[Point]` without private-module access.
...
- Spec coverage: parsing, 110-point count, structured errors, geometry indices/truncation, exact crop constants, audio boundaries, read-only API, tests, and dependency exclusions map to Tasks 1-3.
