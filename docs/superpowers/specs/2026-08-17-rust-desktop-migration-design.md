...
- CLI 覆盖所有 worker 能力，便于自动化测试和无界面运行。当前 `feathertalk` 已覆盖 worker 的全部命令（`capabilities`、`validate-project`、`probe-media`、`normalize-media`），后续每新增一个 worker 命令都必须同步新增子命令。
